...
describe('FilePreview', () => {
  it('shows a placeholder when nothing is selected', () => {
    ...
    render(<FilePreview workspace="/repo" path={null} onOpen={vi.fn()} />)
  ...
  it('renders text with the path and a line-number gutter', async () => {
    ...
    const { container } = render(<FilePreview workspace="/repo" path="src/a.txt" onOpen={vi.fn()} />)
  ...
  it('syntax-highlights a known code language into token spans', async () => {
    ...
    const { container } = render(<FilePreview workspace="/repo" path="src/a.ts" onOpen={vi.fn()} />)
  ...
  it('notes truncation for a capped large file', async () => {
    ...
    render(<FilePreview workspace="/repo" path="big.log" onOpen={vi.fn()} />)
  ...
  it('renders an image as an <img>', async () => {
    ...
    render(<FilePreview workspace="/repo" path="a.png" onOpen={vi.fn()} />)
  ...
  it('falls back to a note for binary files', async () => {
    ...
    render(<FilePreview workspace="/repo" path="a.bin" onOpen={vi.fn()} />)
    expect(await screen.findByText(/Binary file/)).toBeInTheDocument()
  })

  it('falls back to a note for oversize files', async () => {
    installApi({ kind: 'too-large', bytes: 9_000_000, limit: 5_000_000 })
    render(<FilePreview workspace="/repo" path="huge.bin" onOpen={vi.fn()} />)
  ...
  it('surfaces a read error', async () => {
    ...
    render(<FilePreview workspace="/repo" path="x" onOpen={vi.fn()} />)
  ...
  it('renders markdown files, with a Source toggle back to raw text', async () => {
    ...
    const { container } = render(<FilePreview workspace="/repo" path="README.md" onOpen={vi.fn()} />)
  ...
  it('renders an SVG file as an image, toggleable to source', async () => {
    ...
    const { container } = render(<FilePreview workspace="/repo" path="icon.svg" onOpen={vi.fn()} />)
  ...
  it('calls onOpen with the path from the header button', async () => {
    installApi({ kind: 'text', text: 'hi', truncated: false, bytes: 2 })
    const onOpen = vi.fn()
    render(<FilePreview workspace="/repo" path="src/a.ts" onOpen={onOpen} />)
