...
const FILE_TOOLS = new Set(['read_file', 'write_file', 'edit_file', 'multi_edit', 'apply_patch'])
...
/**
 * Files the run has touched, most-recently-first and deduped. Derived from the
 * `path` argument of file-oriented tool calls across the whole log, so it survives
